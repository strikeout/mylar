...
function _getProj(proj, doc, token) {
    if (!proj) {
...
// name of field for which the search happens
// filter a function that will be called on the filter_args provided by the client
// to output filters for the search
// proj: the fields to be included when returning data to the client,
// if not specified, all fields will be included
Meteor.Collection.prototype.search_filter(filter, proj) {
	...
	if (token != null) {
		...
		var handle = self.find(filter).observe({
		    added: function(doc) {
